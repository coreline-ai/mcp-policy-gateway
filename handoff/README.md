# MCP Runtime Policy Gateway Handoff Pack

작성일: 2026-06-30 KST  
목적: 별도 새 프로젝트를 처음부터 시작할 수 있도록 제품 목적, 범위, 아키텍처, 구현 계획, 테스트 기준, 보안 경계를 한 곳에 고정한다.
...
> MCP Runtime Policy Gateway는 LLM client와 target MCP server 사이에 위치해, target MCP의 tool 노출과 호출을 정책으로 통제하고, 정책상 차단 또는 승인 대상으로 분류된 호출을 target에 전달하지 않으며, 모든 결정의 audit/evidence를 남기는 inline MCP policy proxy다.
...
MVP는 `tools/list`와 `tools/call`만 먼저 다룬다.
...
2. target `tools/list`를 수집하고 hash snapshot을 저장한다.
3. 정책상 허용된 tool만 upstream에 노출한다.
4. 숨겨진 tool을 직접 `tools/call`해도 target으로 전달하지 않는다.
5. mutation/destructive tool은 approval 없이는 실행하지 않는다.
6. 모든 allow/block/approval/call/error는 audit id를 남긴다.
7. 공개 demo에는 최소 1개 이상의 target tool alias가 실제 filtered MCP tool로 upstream `tools/list`에 노출된다.
8. `tools/list` pagination, `nextCursor`, `notifications/tools/list_changed`를 고려해 complete snapshot과 diff를 만든다.
9. 브라우저/API/paywall/약관/rate-limit/anti-bot/인증 우회 목적의 target은 등록하지 않는다.
10. stdio MVP identity는 config single principal로만 설명한다.
11. target registration은 privileged config/operator boundary로 취급한다.
12. incomplete snapshot과 unsupported reverse capability는 fail-closed한다.
