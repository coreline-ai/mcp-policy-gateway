...
이 프로젝트의 정본 목적은 **MCP Runtime Policy Gateway**를 만드는 것이다.
...
따라서 이 프로젝트는 target MCP를 직접 등록하는 대신, **Gateway만 MCP client에 등록하고 target MCP는 Gateway 뒤에 둔다.**
...
Gateway는 target MCP의 `tools/list`를 수집하고 정책에 맞는 tool만 upstream client에 노출한다. `tools/call` 시점에는 target 호출 전에 policy engine이 `allow`, `block`, `approval_required`, `rewrite`, `limited_alias` 결정을 내린다.

## 4. Core Differentiation

이 프로젝트의 차별점은 "MCP 보안 검사"가 아니다.
...
| 약한 제품 | 강한 제품 |
|---|---|
| MCP 설정을 분석해 위험 리포트 제공 | MCP tool 노출 자체를 축소 |
| 위험하다고 경고 | 위험 tool 호출을 target에 전달하지 않음 |
| 보안 스캐너 | MCP server/client 양쪽을 모두 구현하는 runtime policy proxy |
| 단발 검사 | tool snapshot, version diff, audit history 유지 |
| 사람이 읽는 리포트 | LLM client가 바로 사용할 filtered MCP surface 제공 |
